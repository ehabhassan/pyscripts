...
def plot_fastran_outputs(fastrandata,png=False,plotparam={}):
...
def fastran_plot(WORK_DIR,png=True,cmpr=False,plotparam={}):
    if cmpr: statedata = {}
    fastrandata = {}
    ...
    for iWORK_DIR in WORK_DIR:
        ...
        if fCURRENT_STATE[WORK_DIR.index(iWORK_DIR)] and cmpr:
    ...
    if any(fCURRENT_FASTRAN):
       fastranplot = plot_fastran_outputs(fastrandata,png=True,plotparam={})

    return fastrandata,fastranplot
    
        
if __name__ == "__main__":
   FASTRAN_DIR = ['220003.00004','220003.00005','220003.00006','220003.00007','220003.00008','220003.00009'] 
   
   WORK_DIR = []
   
   if   len(sys.argv) >= 2:
        for i in range(len(sys.argv[1:])):
            WORK_DIR.append(sys.argv[i+1])
   elif FASTRAN_DIR:
        if type(FASTRAN_DIR) in (list,tuple):
           for i in range(len(FASTRAN_DIR)):
               if os.path.isdir(FASTRAN_DIR[i]):
                  WORK_DIR.append(FASTRAN_DIR[i])
        else:
               WORK_DIR.append(FASTRAN_DIR)
   else:
        raise IOError('SIMULATION FOLDER NOT FOUND ... EXIT!')
   
   fastrandata,fastranplot = fastran_plot(WORK_DIR)
